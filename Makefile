...
$(IPXE_IMG): PKG=ipxe
$(BIOS_IMG): PKG=uefi
$(BIOS_IMG): INSTALLER=$(INSTALLER_FIRMWARE_DIR)
$(EFI_PART) $(BOOT_PART) $(INITRD_IMG) $(INSTALLER_IMG) $(KERNEL_IMG) $(IPXE_IMG) $(BIOS_IMG): $(LINUXKIT) | $(INSTALLER) $(INSTALLER_FIRMWARE_DIR)
	cd $(INSTALLER) ; $(DOCKER_UNPACK) $(shell $(LINUXKIT) pkg show-tag pkg/$(PKG))-$(DOCKER_ARCH_TAG) $(notdir $@)
	$(QUIET): $@: Succeeded
...
$(INSTALLER):
	@mkdir -p $@
	@cp -r images/*.yml pkg/eve/installer/* $@
